...
default_args = {
    ...
    # 'email': ['henri.gohi@utt.fr', 'christian_olivier.assienne_boyoguino@utt.fr'],
...
def get_next_api_key():
    ...
    for key in serpapi_keys_str.split(','):
        if key.strip():
    ...
    current_index = Variable.get('serpapi_current_key_index', default_var=0)
...
def get_next_city():
    ...
    for city in city_id_str.split(','):
        if city.strip():
            city_ids.append(city.strip())
    ...
    current_index = Variable.get('city_current_id_index', default_var=0)
    current_index = int(current_index)
...
def download_flight_data(**kwargs):
    # SERPAPI_KEYS = list(os.getenv('SERPAPI_KEYS'))
